...
export default function SelectWithSearchTourType({  selected, onSelect, reset }) {
  ...
  useEffect(() => {
    if (token) {
      fetch(`http://localhost:9092/data/admin/getTourTypes/${token}`, {
        method: 'GET',
        headers: {
    ...
    <div className="select js-select js-liveSearch" data-select-value="">
      ...
      >
        <span className="js-button-title">
          {selectedItem ? selectedItem : `Tour Type`}
        </span>
      ...
      >
        ...
        <div className="select__options js-options">
          {tourTypes
            ?.filter((elm) =>
              elm.type?.toLowerCase().includes(searchQuery?.toLowerCase()),
            )
            .map((elm) => (
              <div
                onClick={() => {
                  setSelectedItem(elm.type);
                  setDdActive(false);
                  onSelect(elm.id)
                }}
